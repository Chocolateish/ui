...
export class FormCode extends FormBaseWrite<string> {
  ...
  set live(live: boolean | undefined) {
    if (live) this._input.oninput = () => this._inputUpdate();
    else {
      this._input.onchange = () => this._inputUpdate();
      this._input.oninput = () => {
        this._unitSpacer.textContent = this._input.value;
      };
    }
  ...
  private _inputUpdate() {
    switch (this._type) {
      case FormInputType.number:
        ...
        break;
      case FormInputType.text:
        this._valueSet(this._input.value as InputTypeMap[T]);
        break;
    }
  }
}
